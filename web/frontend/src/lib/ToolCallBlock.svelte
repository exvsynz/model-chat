<script lang="ts">
    ...
    const borderClass = $derived(
        block.status === 'running' ? 'border-blue-400/50 dark:border-blue-500/40' :
        block.status === 'error' ? 'border-red-400/50 dark:border-red-500/40 bg-red-50/50 dark:bg-red-950/20' :
        'border-zinc-300 dark:border-zinc-700'
    );

    const iconClass = $derived(
        block.status === 'running' ? 'text-blue-500 animate-spin' :
        block.status === 'success' ? 'text-green-500' :
        'text-red-500'
    );
</script>
...
<div class="my-2 rounded-lg border {borderClass} text-sm overflow-hidden">
    ...
    >
        ...
        <span class="font-mono text-xs text-zinc-600 dark:text-zinc-400 flex-1 truncate">{block.name}</span>
        <span class="text-zinc-400 dark:text-zinc-500 text-xs flex-shrink-0 transition-transform {expanded ? 'rotate-90' : ''}">▶</span>
    </button>

    {#if expanded}
        <div class="border-t border-zinc-200 dark:border-zinc-700/50 px-3 py-2 space-y-2">
            {#if Object.keys(block.arguments).length > 0}
                <pre class="text-xs text-zinc-500 dark:text-zinc-400 whitespace-pre-wrap break-all font-mono leading-relaxed">{formatArgs(block.arguments)}</pre>
            {/if}

            {#if block.output != null}
                <div class="border-t border-zinc-200 dark:border-zinc-700/50 pt-2">
                    <pre class="text-xs whitespace-pre-wrap break-all font-mono leading-relaxed {block.is_error ? 'text-red-600 dark:text-red-400' : 'text-zinc-700 dark:text-zinc-300'}">{displayOutput}</pre>
                    {#if isTruncated && !showFullOutput}
                        <button
                            class="text-xs text-blue-500 hover:text-blue-400 mt-1"
                            onclick={(e: MouseEvent) => { e.stopPropagation(); showFullOutput = true; }}
                        >
