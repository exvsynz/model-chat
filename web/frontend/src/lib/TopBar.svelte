...
<div
	class="flex items-center gap-2 md:gap-4 px-2 md:px-4 py-2 bg-zinc-100 dark:bg-zinc-800 border-b border-zinc-300 dark:border-zinc-700"
>
	<button
		onclick={onToggleSidebar}
		class="md:hidden p-2 rounded text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700"
	...
	<div class="relative flex-1 md:flex-none">
		...
		{#if showDropdown}
			<div
				class="absolute top-full left-0 mt-1 w-[calc(100vw-1rem)] md:w-96 max-h-80 overflow-y-auto bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded-lg shadow-xl z-50"
			>
				{#each filteredModels as model}
					<button
						onmousedown={() => selectModel(model.id)}
						class="w-full text-left px-3 py-2 text-sm hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors {model.id ===
						currentModel
							? 'bg-zinc-200 dark:bg-zinc-700 text-blue-400'
	...
	<div class="hidden sm:flex gap-1">
		{#each ['low', 'medium', 'high'] as level}
			<button
				class="px-2 py-1 text-xs rounded {currentEffort === level
